...
function getservice($lat,$lgt)
{
    $key = Config::get('webservice.key');
    $url = 'https://apis.map.qq.com/ws/geocoder/v1/?location='."$lat".','."$lgt".'&key=' . "$key" . '&get_poi=1';
    $list = json_decode(curl_get($url), true);
    return $list;
}
...
function address($address)
{
    ...
    if(array_key_exists('result', $list)){
